...
{
    public partial class ctrlPaymentMethods : UserControl
    {
        private static class Columns
        {
            public const string PaymentMethodID= nameof(clsPaymentMethodsDto.PaymentMethodID);
            public const string MethodName = nameof(clsPaymentMethodsDto.MethodName);
        }
        ...
        private readonly clsPaymentMethodsService _PaymentMethodservice;
        ...
        public ctrlPaymentMethods(frmMain frmMain)
        {
            InitializeComponent();

            clsPermissionHelper.ApplyPermissions(this);

            _PaymentMethodservice = new clsPaymentMethodsService();
        ...
        private async void ctrlPaymentMethods_Load(object sender, EventArgs e)
        {
            try
            {
            ...
            {
                clsEventLogger.LogException("ctrlPaymentMethods.ctrlPaymentMethods_Load", ex);
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            ...
            {
                clsMessages.ShowError("ليس لديك صلاحية تصدير بيانات طرق الدفع");
                return;
            }
        ...
        private async void txtSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlPaymentMethods.txtSearch_TextChanged", ex);
                clsMessages.ShowError();
            }
        }
        private async void txtSearch_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlPaymentMethods.txtSearch_KeyPress", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnRefresh_Click(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlPaymentMethods.btnRefresh_Click", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlPaymentMethods.btnSearch_Click", ex);
                clsMessages.ShowError();
            }
        }
        ...
        private async void cbPageNumber_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlPaymentMethods.cbPageNumber_SelectedIndexChanged", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnPrevious_Click(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlPaymentMethods.btnPrevious_Click", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnNext_Click(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlPaymentMethods.btnNext_Click", ex);
                clsMessages.ShowError();
            }
        }
        ...
        private async Task _LoadDataAsync()
        {
            ...
            using (var loading = new frmLoading())
            {
                ...
                catch (Exception ex)
                {
                    ...
                    clsEventLogger.LogException("ctrlPaymentMethods.LoadDataAsync", ex);
                    _ShowServerErrorState();
                }
                finally
                {
        ...
        private async Task _ApplyPagingAsync()
        {
            try
            {
                var result = await _PaymentMethodservice.GetPaymentMethodsPageAsync(
                    _currentPage,
                ...
                {
                    dgvListPaymentMethods.DataSource = null;
                ...
                dgvListPaymentMethods.DataSource = result.Data.Data;

                _UpdatePagingControls(newTotalPages);

                if (!_columnsInitialized && dgvListPaymentMethods.Rows.Count > 0)
                {
                    _InitializeColumns();
            ...
            {
                clsEventLogger.LogException("ctrlPaymentMethods._ApplyPagingAsync", ex);
                _ShowServerErrorState();
            }
        }
        ...
        {
            if (dgvListPaymentMethods.DataSource == null || dgvListPaymentMethods.Rows.Count == 0)
                return;

            _SetColumnHeader(Columns.PaymentMethodID           , "المعرف"      );
            _SetColumnHeader(Columns.MethodName	  	  , "طريقة الدفع"  );

        }
        private void _SetColumnHeader(string columnName, string headerText)
        {
            if (dgvListPaymentMethods.Columns.Contains(columnName))
                dgvListPaymentMethods.Columns[columnName].HeaderText = headerText;
        }
        private void _ShowEmptyDataState()
        {
            bool isEmpty = dgvListPaymentMethods.Rows.Count == 0;
        ...
        {
            var data = dgvListPaymentMethods.DataSource as List<clsPaymentMethodsDto>;
            ...
            clsExcelHelper.Export(_frmMain, optimizedData, "طرق الدفع");
        }
        private DataTable _CreateOptimizedExportData(List<clsPaymentMethodsDto> source)
        {
            ...
            foreach (var item in source)
            {
                ...
                newRow["المعرف"            ] = item.PaymentMethodID;
                newRow["طريقة الدفع"        ] = item.MethodName;
        ...
        private bool _TryGetSelectedPaymentMethodId(out int PaymentMethodId)
        {
            PaymentMethodId = 0;
            if (dgvListPaymentMethods.CurrentRow == null)
            {
                clsMessages.ShowError("الرجاء اختيار صف أولا");
                return false;
            }

            if (dgvListPaymentMethods.CurrentRow.DataBoundItem is clsPaymentMethodsDto PaymentMethod)
            {
        ...
        private string _GetColumnNameForFilter(enFilter filter)
        {
            switch (filter)
            {
                case enFilter.PaymentMethodID: return Columns.PaymentMethodID;
