...
namespace CarRental.Payments.PaymentStatus.Controls
{
    public partial class ctrlPaymentStatus : UserControl
    {
        private static class Columns
        {
            public const string PaymentStatusID = nameof(clsPaymentStatusDto.PaymentStatusID);
            public const string StatusName = nameof(clsPaymentStatusDto.StatusName);
        }
        ...
        private readonly clsPaymentStatusService _PaymentStatuservice;
        ...
        public ctrlPaymentStatus(frmMain frmMain)
        {
            InitializeComponent();

            clsPermissionHelper.ApplyPermissions(this);

            _PaymentStatuservice = new clsPaymentStatusService();
        ...
        private async void ctrlPaymentStatus_Load(object sender, EventArgs e)
        {
            try
            {
            ...
            {
                clsEventLogger.LogException("ctrlPaymentStatus.ctrlPaymentStatus_Load", ex);
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            ...
            {
                clsMessages.ShowError("ليس لديك صلاحية تصدير بيانات حالات الدفع");
                return;
            }
        ...
        private async void txtSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlPaymentStatus.txtSearch_TextChanged", ex);
                clsMessages.ShowError();
            }
        }
        private async void txtSearch_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlPaymentStatus.txtSearch_KeyPress", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnRefresh_Click(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlPaymentStatus.btnRefresh_Click", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlPaymentStatus.btnSearch_Click", ex);
                clsMessages.ShowError();
            }
        }
        ...
        private async void cbPageNumber_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlPaymentStatus.cbPageNumber_SelectedIndexChanged", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnPrevious_Click(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlPaymentStatus.btnPrevious_Click", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnNext_Click(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlPaymentStatus.btnNext_Click", ex);
                clsMessages.ShowError();
            }
        }
        ...
        private async Task _LoadDataAsync()
        {
            ...
            using (var loading = new frmLoading())
            {
                ...
                catch (Exception ex)
                {
                    ...
                    clsEventLogger.LogException("ctrlPaymentStatus.LoadDataAsync", ex);
                    _ShowServerErrorState();
                }
                finally
                {
        ...
        private async Task _ApplyPagingAsync()
        {
            try
            {
                var result = await _PaymentStatuservice.GetPaymentStatusPageAsync(
                    _currentPage,
                ...
                {
                    dgvListPaymentStatus.DataSource = null;
                ...
                dgvListPaymentStatus.DataSource = result.Data.Data;

                _UpdatePagingControls(newTotalPages);

                if (!_columnsInitialized && dgvListPaymentStatus.Rows.Count > 0)
                {
                    _InitializeColumns();
            ...
            {
                clsEventLogger.LogException("ctrlPaymentStatus._ApplyPagingAsync", ex);
                _ShowServerErrorState();
            }
        }
        ...
        {
            if (dgvListPaymentStatus.DataSource == null || dgvListPaymentStatus.Rows.Count == 0)
                return;

            _SetColumnHeader(Columns.PaymentStatusID     , "المعرف"      );
            _SetColumnHeader(Columns.StatusName	  	  , "حالة الدفع"  );

        }
        private void _SetColumnHeader(string columnName, string headerText)
        {
            if (dgvListPaymentStatus.Columns.Contains(columnName))
                dgvListPaymentStatus.Columns[columnName].HeaderText = headerText;
        }
        private void _ShowEmptyDataState()
        {
            bool isEmpty = dgvListPaymentStatus.Rows.Count == 0;
        ...
        {
            var data = dgvListPaymentStatus.DataSource as List<clsPaymentStatusDto>;
            ...
            clsExcelHelper.Export(_frmMain, optimizedData, "حالات الدفع");
        }
        private DataTable _CreateOptimizedExportData(List<clsPaymentStatusDto> source)
        {
            var exportTable = new DataTable();

            exportTable.Columns.Add("المعرف"            ,typeof(int));
            exportTable.Columns.Add("حالة الدفع"        , typeof(string));
            ...
            foreach (var item in source)
            {
                ...
                newRow["المعرف"            ] = item.PaymentStatusID;
                newRow["حالة الدفع"        ] = item.StatusName;
        ...
        private bool _TryGetSelectedPaymentStatusID(out int PaymentStatusID)
        {
            PaymentStatusID = 0;
            if (dgvListPaymentStatus.CurrentRow == null)
            {
                clsMessages.ShowError("الرجاء اختيار صف أولا");
                return false;
            }

            if (dgvListPaymentStatus.CurrentRow.DataBoundItem is clsPaymentStatusDto PaymentMethod)
            {
                PaymentStatusID = PaymentMethod.PaymentStatusID;
                return true;
            }
            return false;
        }
        ...
        private string _GetColumnNameForFilter(enFilter filter)
        {
            switch (filter)
            {
                case enFilter.PaymentStatusID: return Columns.PaymentStatusID;
