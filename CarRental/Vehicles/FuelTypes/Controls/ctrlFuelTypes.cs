...
using CarRental_Buisness.Services.FuelTypes;
using SharedClass;
using System;
...
namespace CarRental.Vehicles.FuelTypes.Controls
{
    public partial class ctrlFuelTypes : UserControl
    {
        private static class Columns
        {
            public const string FuelTypeID= nameof(clsFuelTypeDto.FuelTypeID);
            public const string FuelTypeName = nameof(clsFuelTypeDto.FuelTypeName);
        }
        ...
        private readonly clsFuelTypeService _FuelTypeservice;
        ...
        public ctrlFuelTypes(frmMain frmMain)
        {
            InitializeComponent();

            clsPermissionHelper.ApplyPermissions(this);

            _FuelTypeservice = new clsFuelTypeService();
        ...
        private async void ctrlFuelTypes_Load(object sender, EventArgs e)
        {
            try
            {
            ...
            {
                clsEventLogger.LogException("ctrlFuelTypes.ctrlFuelTypes_Load", ex);
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            ...
            {
                clsMessages.ShowError("ليس لديك صلاحية تصدير بيانات أنواع الوقود");
            }
            _ExportToExcel();
        }
        private async void txtSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlFuelTypes.txtSearch_TextChanged", ex);
                clsMessages.ShowError();
            }
        }
        private async void txtSearch_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlFuelTypes.txtSearch_KeyPress", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnRefresh_Click(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlFuelTypes.btnRefresh_Click", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlFuelTypes.btnSearch_Click", ex);
                clsMessages.ShowError();
            }
        }
        ...
        private async void cbPageNumber_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlFuelTypes.cbPageNumber_SelectedIndexChanged", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnPrevious_Click(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlFuelTypes.btnPrevious_Click", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnNext_Click(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlFuelTypes.btnNext_Click", ex);
                clsMessages.ShowError();
            }
        }
        ...
        private async Task _LoadDataAsync()
        {
            ...
            using (var loading = new frmLoading())
            {
                ...
                catch (Exception ex)
                {
                    ...
                    clsEventLogger.LogException("ctrlFuelTypes.LoadDataAsync", ex);
                    _ShowServerErrorState();
                }
                finally
                {
        ...
        private async Task _ApplyPagingAsync()
        {
            try
            {
                var result = await _FuelTypeservice.GetFuelTypePageAsync(
                    _currentPage,
                ...
                {
                    dgvListFuelTypes.DataSource = null;
                ...
                dgvListFuelTypes.DataSource = result.Data.Data;

                _UpdatePagingControls(newTotalPages);

                if (!_columnsInitialized && dgvListFuelTypes.Rows.Count > 0)
                {
                    _InitializeColumns();
            ...
            {
                clsEventLogger.LogException("ctrlFuelTypes._ApplyPagingAsync", ex);
                _ShowServerErrorState();
            }
        }
        ...
        {
            if (dgvListFuelTypes.DataSource == null || dgvListFuelTypes.Rows.Count == 0)
                return;

            _SetColumnHeader(Columns.FuelTypeID           , "المعرف"      );
            _SetColumnHeader(Columns.FuelTypeName	  	  , "نوع الوقود"  );

        }
        private void _SetColumnHeader(string columnName, string headerText)
        {
            if (dgvListFuelTypes.Columns.Contains(columnName))
                dgvListFuelTypes.Columns[columnName].HeaderText = headerText;
        }
        private void _HideColumn(string columnName)
        {
            if (dgvListFuelTypes.Columns.Contains(columnName))
                dgvListFuelTypes.Columns[columnName].Visible = false;
        }
        private void _ShowEmptyDataState()
        {
            bool isEmpty = dgvListFuelTypes.Rows.Count == 0;
        ...
        {
            var data = dgvListFuelTypes.DataSource as List<clsFuelTypeDto>;
            ...
            clsExcelHelper.Export(_frmMain, optimizedData, "أنواع الوقود");
        }
        private DataTable _CreateOptimizedExportData(List<clsFuelTypeDto> source)
        {
            ...
            foreach (var item in source)
            {
                ...
                newRow["المعرف"            ] = item.FuelTypeID;
                newRow["نوع الوقود"        ] = item.FuelTypeName;
        ...
        private bool _TryGetSelectedFuelTypeId(out int FuelTypeId)
        {
            FuelTypeId = 0;
            if (dgvListFuelTypes.CurrentRow == null)
            {
                clsMessages.ShowError("الرجاء اختيار صف أولا");
                return false;
            }

            if (dgvListFuelTypes.CurrentRow.DataBoundItem is clsFuelTypeDto FuelType)
            {
        ...
        private string _GetColumnNameForFilter(enFilter filter)
        {
            switch (filter)
            {
                case enFilter.FuelTypeID: return Columns.FuelTypeID;
