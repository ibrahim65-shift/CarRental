...
{
    public partial class ctrlVehicleStatus : UserControl
    {
        private static class Columns
        {
            public const string StatusID = nameof(clsVehicleStatusDto.StatusID);
            public const string StatusName = nameof(clsVehicleStatusDto.StatusName);
        }
        ...
        private readonly clsVehicleStatusService _VehicleStatuservice;
        ...
        private async void ctrlVehicleStatus_Load(object sender, EventArgs e)
        {
            try
            {
            ...
            {
                clsEventLogger.LogException("ctrlVehicleStatus.ctrlVehicleStatus_Load", ex);
        ...
        private async void txtSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlVehicleStatus.txtSearch_TextChanged", ex);
                clsMessages.ShowError();
            }
        }
        private async void txtSearch_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlVehicleStatus.txtSearch_KeyPress", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnRefresh_Click(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlVehicleStatus.btnRefresh_Click", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlVehicleStatus.btnSearch_Click", ex);
                clsMessages.ShowError();
            }
        }
        ...
        private async void cbPageNumber_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlVehicleStatus.cbPageNumber_SelectedIndexChanged", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnPrevious_Click(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlVehicleStatus.btnPrevious_Click", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnNext_Click(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlVehicleStatus.btnNext_Click", ex);
                clsMessages.ShowError();
            }
        }
        ...
        private async Task _LoadDataAsync()
        {
            ...
            using (var loading = new frmLoading())
            {
                ...
                catch (Exception ex)
                {
                    ...
                    clsEventLogger.LogException("ctrlVehicleStatus.LoadDataAsync", ex);
                    _ShowServerErrorState();
                }
                finally
                {
        ...
        private async Task _ApplyPagingAsync()
        {
            try
            {
                var result = await _VehicleStatuservice.GetVehicleStatusPageAsync(
                    _currentPage,
                ...
                {
                    dgvListVehicleStatus.DataSource = null;
                ...
                dgvListVehicleStatus.DataSource = result.Data.Data;

                _UpdatePagingControls(newTotalPages);

                if (!_columnsInitialized && dgvListVehicleStatus.Rows.Count > 0)
                {
                    _InitializeColumns();
            ...
            {
                clsEventLogger.LogException("ctrlVehicleStatus._ApplyPagingAsync", ex);
                _ShowServerErrorState();
            }
        }
        ...
        {
            if (dgvListVehicleStatus.DataSource == null || dgvListVehicleStatus.Rows.Count == 0)
                return;

            _SetColumnHeader(Columns.StatusID           , "المعرف"      );
            _SetColumnHeader(Columns.StatusName	  	  , "الحالة"  );

        }
        private void _SetColumnHeader(string columnName, string headerText)
        {
            if (dgvListVehicleStatus.Columns.Contains(columnName))
                dgvListVehicleStatus.Columns[columnName].HeaderText = headerText;
        }
        private void _HideColumn(string columnName)
        {
            if (dgvListVehicleStatus.Columns.Contains(columnName))
                dgvListVehicleStatus.Columns[columnName].Visible = false;
        }
        private void _ShowEmptyDataState()
        {
            bool isEmpty = dgvListVehicleStatus.Rows.Count == 0;
        ...
        {
            var data = dgvListVehicleStatus.DataSource as List<clsVehicleStatusDto>;
            ...
            clsExcelHelper.Export(_frmMain, optimizedData, "حالات المركبة");
        }
        private DataTable _CreateOptimizedExportData(List<clsVehicleStatusDto> source)
        {
        ...
        private bool _TryGetSelectedVehicleStatusId(out int VehicleStatusId)
        {
            VehicleStatusId = 0;
            if (dgvListVehicleStatus.CurrentRow == null)
            {
                clsMessages.ShowError("الرجاء اختيار صف أولا");
                return false;
            }

            if (dgvListVehicleStatus.CurrentRow.DataBoundItem is clsVehicleStatusDto VehicleStatus)
            {
                VehicleStatusId = VehicleStatus.StatusID;
                return true;
            }
            return false;
        }
        ...
        private string _GetColumnNameForFilter(enFilter filter)
        {
            switch (filter)
            {
                case enFilter.StatusID: return Columns.StatusID;
