...
using CarRental_Buisness.Services.ReturnStatus;
using SharedClass;
using System;
...
{
    public partial class ctrlReturnStatus : UserControl
    {
        private static class Columns
        {
            public const string ReturnStatusID= nameof(clsReturnStatusDto.ReturnStatusID);
            public const string StatusName = nameof(clsReturnStatusDto.StatusName);
        }

        private enum enFilter
        {
            ReturnStatusID, StatusName
        }
        ...
        private readonly clsReturnStatusService _ReturnStatuservice;
        ...
        private async void ctrlReturnStatus_Load(object sender, EventArgs e)
        {
            try
            {
            ...
            {
                clsEventLogger.LogException("ctrlReturnStatus.ctrlReturnStatus_Load", ex);
        ...
        private async void txtSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlReturnStatus.txtSearch_TextChanged", ex);
                clsMessages.ShowError();
            }
        }
        private async void txtSearch_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlReturnStatus.txtSearch_KeyPress", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnRefresh_Click(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlReturnStatus.btnRefresh_Click", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlReturnStatus.btnSearch_Click", ex);
                clsMessages.ShowError();
            }
        }
        ...
        private async void cbPageNumber_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlReturnStatus.cbPageNumber_SelectedIndexChanged", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnPrevious_Click(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlReturnStatus.btnPrevious_Click", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnNext_Click(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlReturnStatus.btnNext_Click", ex);
                clsMessages.ShowError();
            }
        }
        ...
        private async Task _LoadDataAsync()
        {
            ...
            using (var loading = new frmLoading())
            {
                ...
                catch (Exception ex)
                {
                    ...
                    clsEventLogger.LogException("ctrlReturnStatus.LoadDataAsync", ex);
                    _ShowServerErrorState();
                }
                finally
                {
        ...
        private async Task _ApplyPagingAsync()
        {
            try
            {
                var result = await _ReturnStatuservice.GetReturnStatusPageAsync(
                    _currentPage,
                ...
                {
                    dgvListReturnStatus.DataSource = null;
                ...
                dgvListReturnStatus.DataSource = result.Data.Data;

                _UpdatePagingControls(newTotalPages);

                if (!_columnsInitialized && dgvListReturnStatus.Rows.Count > 0)
                {
                    _InitializeColumns();
            ...
            {
                clsEventLogger.LogException("ctrlReturnStatus._ApplyPagingAsync", ex);
                _ShowServerErrorState();
            }
        }
        ...
        {
            if (dgvListReturnStatus.DataSource == null || dgvListReturnStatus.Rows.Count == 0)
                return;

            _SetColumnHeader(Columns.ReturnStatusID           , "المعرف"      );
            _SetColumnHeader(Columns.StatusName	  	  , "الحالة"  );

        }
        private void _SetColumnHeader(string columnName, string headerText)
        {
            if (dgvListReturnStatus.Columns.Contains(columnName))
                dgvListReturnStatus.Columns[columnName].HeaderText = headerText;
        }
        private void _ShowEmptyDataState()
        {
            bool isEmpty = dgvListReturnStatus.Rows.Count == 0;
        ...
        {
            var data = dgvListReturnStatus.DataSource as List<clsReturnStatusDto>;
            ...
            clsExcelHelper.Export(_frmMain, optimizedData, "حالات الإرجاع");
        }
        private DataTable _CreateOptimizedExportData(List<clsReturnStatusDto> source)
        {
            ...
            foreach (var item in source)
            {
                ...
                newRow["المعرف"            ] = item.ReturnStatusID;
                newRow["الحالة"        ] = item.StatusName;
        ...
        private bool _TryGetSelectedReturnStatusId(out int ReturnStatusId)
        {
            ReturnStatusId = 0;
            if (dgvListReturnStatus.CurrentRow == null)
            {
                clsMessages.ShowError("الرجاء اختيار صف أولا");
                return false;
            }

            if (dgvListReturnStatus.CurrentRow.DataBoundItem is clsReturnStatusDto ReturnStatus)
            {
                ReturnStatusId = ReturnStatus.ReturnStatusID;
                return true;
            }
            return false;
        }
        ...
        private string _GetColumnNameForFilter(enFilter filter)
        {
            switch (filter)
            {
                case enFilter.ReturnStatusID: return Columns.ReturnStatusID;
