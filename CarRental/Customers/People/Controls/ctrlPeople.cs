...
{
    public partial class ctrlPeople : UserControl, IRefreshable
    {
        private static class Columns
        {
            public const string PersonID = "PersonID";
            public const string NationalNo = "NationalNo";
        ...
        {
            PersonID, NationalNo, FullName, Age,
        ...
        private async void ctrlPeople_Load(object sender, EventArgs e)
        {
            try
            {
            ...
            {
                clsEventLogger.LogException("ctrlPeople.ctrlPeople_Load", ex);
        ...
        private async void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlPeople.btnAdd_Click", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnEdit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (frmAddEditPerson frm = new frmAddEditPerson(_personService,PersonId))
                {
                    if (frm.ShowDialog() == DialogResult.OK)
                    {
                        await RefreshDataAsync();
            ...
            {
                clsEventLogger.LogException("ctrlPeople.btnEdit_Click", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (result.Success)
                {
                    clsMessages.ShowSuccess($"تم حذف الشخص الذي يحمل الرقم التعريفي '{PersonId}' بنجاح");
                    await RefreshDataAsync();
                    DataRefreshed?.Invoke();
                }
                else
            ...
            {
                clsMessages.ShowError();
                clsEventLogger.LogException("ctrlPeople.btnDelete_Click", ex);
            }
        }
        ...
        private async void txtSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlPeople.txtSearch_TextChanged", ex);
                clsMessages.ShowError();
            }
        }
        private async void txtSearch_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlPeople.txtSearch_KeyPress", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnRefresh_Click(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlPeople.btnRefresh_Click", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlPeople.btnSearch_Click", ex);
                clsMessages.ShowError();
            }
        }
        ...
        private async void cbPageNumber_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlPeople.cbPageNumber_SelectedIndexChanged", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnPrevious_Click(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlPeople.btnPrevious_Click", ex);
                clsMessages.ShowError();
            }
        }
        private async void btnNext_Click(object sender, EventArgs e)
        {
            ...
            {
                clsEventLogger.LogException("ctrlPeople.btnNext_Click", ex);
                clsMessages.ShowError();
            }
        }
        ...
        private async Task _LoadDataAsync()
        {
            ...
            using (var loading = new frmLoading())
            {
                ...
                catch (Exception ex)
                {
                    ...
                    clsEventLogger.LogException("ctrlPeople.LoadDataAsync", ex);
                    _ShowServerErrorState();
                }
                finally
                {
        ...
        private async Task _ApplyPagingAsync()
        {
            try
            {
                var result = await _personService.GetPeoplePageAsync(
                    _currentPage,
                ...
                dgvListPeople.DataSource = result.Data.Data;

                _UpdatePagingControls(newTotalPages);

                if (!_columnsInitialized && dgvListPeople.Rows.Count > 0)
                {
                    _InitializeColumns();
            ...
            {
                clsEventLogger.LogException("ctrlPeople._ApplyPagingAsync", ex);
                _ShowServerErrorState();
            }
        }
        ...
        {
            if (dgvListPeople.DataSource == null || dgvListPeople.Rows.Count == 0)
                return;

            _SetColumnHeader(Columns.PersonID, "المعرف");
        ...
        {
            if (dgvListPeople.Columns.Contains(columnName))
                dgvListPeople.Columns[columnName].HeaderText = headerText;
        }
        private void _HideColumn(string columnName)
        {
            if (dgvListPeople.Columns.Contains(columnName))
                dgvListPeople.Columns[columnName].Visible = false;
        }
        private void _ShowEmptyDataState()
        {
            bool isEmpty = dgvListPeople.Rows.Count == 0;
        ...
        {
            var data = dgvListPeople.DataSource as DataTable;
        ...
        private DataTable _CreateOptimizedExportData(DataTable source)
        {
            ...
            exportTable.Columns.Add("محذوف", typeof(string));
            ...
            foreach (DataRow row in source.Rows)
            {
                ...
                newRow["المعرف"] = row[Columns.PersonID];
                newRow["الهوية"] = row[Columns.FullName];
        ...
        private bool _TryGetSelectedPersonId(out int PersonId)
        {
            ...
            return _TryGetCellIntValue(row, Columns.PersonID, out PersonId);
        }
        private bool _TryGetSelectedRow(out DataGridViewRow row)
        {
            row = dgvListPeople.CurrentRow;
        ...
        private string _GetColumnNameForFilter(enFilter filter)
        {
            switch (filter)
            {
                case enFilter.PersonID: return Columns.PersonID;
                case enFilter.NationalNo: return Columns.NationalNo;
